...
class DrawScene(QGraphicsScene):
    ...
    def mousePressEvent(self, event): 
        ...
        if self._mode == Utils.BoardSceneMode.DrawLineMode: # Note DrawLineMode lets us draw sequential lines, while draw(Rect,Ellipse)Utils.BoardSceneMode only lets us draw one shape at a time. This is bc drawing one line at a time is a pain
            ...
            self._line = NonConnectivityLineItem( self.activeLayer() , QLineF(self.startPos, self.startPos))
            ...
            if not self._rect: 
                if self.activeLayer() in Utils.copperLayers:
                    # self._rect =NonConnectivityRectItem([self.activeLayer()] , 1)
                ...
                # self._rect = DrawnRectItem( self.activeLayer() , QRectF(self.startPos, self.startPos))
                    self._rect = ConnectivityRectItem( [self.activeLayer()] , 1, QRectF(self.startPos , self.startPos))
                elif self.activeLayer() not in Utils.copperLayers:
                    self._rect = NonConnectivityRectItem( [ self.activeLayer()] , 0, QRectF(self.startPos, self.startPos))
            ...
            if not self._ellipse: 
                ...
                self._ellipse = NonConnectivityEllipseItem([self.activeLayer()] , QRectF(self.startPos , self.startPos))
