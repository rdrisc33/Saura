...
class KicadSymbolConverter(): # Convert .kicad_sym objects into .xml objects. 
# """Converts .kicad_sym file into .sym file"""
    ...
    def __init__(self, libraryPath): 
        if not os.path.exists(libraryPath):  # Verify libraryPath exists
            raise FileNotFoundError(f"SELF.LIBRARY_PATH: {libraryPath} DOES NOT EXIST")
        self.libraryPath = libraryPath
        self.symbolList = self.toList()
        
    @classmethod
    def convert(cls, libraryPath, categories= ['']):
        c = cls(libraryPath) # Create instance
    ...
    # Usage 
    # new_file = MyKicadSymbolConverter.convert(libraryPath)

        
        
        
    def toList(self):
        with open(self.libraryPath) as fo: #fo as in libraryObject
            lines = fo.readlines()
        ...
        symbolList = sexpr_list[3:] # remove sexpr headers
        return symbolList # sexpr file, serialized to list, with headers chopped off, so its a list of the symbols
        
    def symbol_to_xml(self, library_id=None ):
        if library_id == None: 
            _ , lib = os.path.split(self.libraryPath)
        ...
        for symbol in self.symbolList: 
            ...
            if lib_id_normalized == library_id_normalized: 
                ...
                return True
        # print(f'COULD NOT FIND LIBRARY_ID {self.library_id} IN LIBRARY: {self.libraryPath}')
        return False
    
    def save(self, categories:list):
        # print('SAVING')
...
# I popped any handled strings, so only unhandled strings are left
    ...
    # Grab attributes from nested 'symbol' element, set them as attributes on the root level 'library_id'(eg STM32t6) elements
        ...
        for graphic_elem in self.kicad_sym.iterdescendants("graphic"): 
            ...
            self.formatPins(graphic_elem)
            self.formatPolylines(graphic_elem)
            self.formatRectangles(graphic_elem )
        # print('FORMATTED XML:')
        return etree.tostring(self.sym , encoding = str, pretty_print=True)
            
    def formatPins(self, graphic_elem): #Format pins how I want pins formatted 
        # print()
        # print('FORMAT_PINS:')
...
#   </rectangle>
    ...
    def formatRectangles(self, graphic_elem):
    ...
    def formatPolylines(self, graphic_elem):
...
### TESTING ###
# libraryPath = os.path.join('third_party', 'kicad', 'symbols','GRM21BR61E106KA73L.kicad_sym')
# libraryPath = os.path.join('third_party', 'kicad', 'symbols', 'GND.kicad_sym')
# libraryPath = os.path.join('third_party', 'kicad', 'symbols','MIC33153YHJ_TR.kicad_sym')
# print(libraryPath)

# new_file = KicadSymbolConverter.convert(libraryPath)
