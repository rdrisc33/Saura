...
# class TraceItem(NonConnectivityItem, TraceBase, QGraphicsItem): 
...
# class Trace(TraceBase, ConnectivityItem, QGraphicsItem): 
...
#         # super().mousePressEvent(event)This handles selection but leave out for now, while debugging adjust algo
#         self.l0 = self.line()
...
#         if self.l3 != self.l0: # Then we have officially adjusted l0
#             self.adjusted = True # Set adjusted flag 
...
class Trace(ConnectivityItem, QGraphicsLineItem): 
    def __init__(self, layers, traceWidth, net=Net(), parent=None, *args, **kwargs ): 
        print('TRACE.ARGS:', args)
...
# Trace(layers, traceWidth, )
        ...
        self.setLayers(layers)
        self.setTraceWidth(traceWidth)
        self.setColor(Utils.layerColors[layers[0]])
        # self.setPen(QPen(self.pen().color() , self.pen().width() , c = Qt.PenCapStyle.RoundCap)) # Set a round pen cap
...
# def __init__(self, layers, traceWidth, net=Net(), parent=None, *args, **kwargs ): 
    @classmethod 
    def fromLine(cls, line, traceWidth , layers, net = Net()):
        return cls( line, layers=layers, traceWidth=traceWidth, net=net)
    ...
    def setTraceWidth(self, traceWidth): 
        self.prepareGeometryChange() 
        self._traceWidth = traceWidth
        self.setPen(QPen(self.pen().color() , traceWidth, c = Qt.PenCapStyle.RoundCap))
        self.setBufferDistance(self._traceWidth)
...
# add TraceItems to scene, if we were adjusting a trace, based on li123.
        if (self.l1 is not None) and (not self.l1.isNull()):
            if self.li1.line().isNull(): print('WARNING LI1 IS NULL')
            t1 = Trace(self.layers() , self.traceWidth(), self.net(), None, self.li1.line()) # not out of l1, but out of 1i1.line()
            self.scene().addItem(t1)
            
        if (self.l2 is not None) and (not self.l2.isNull()):
            if self.li2.line().isNull(): print('WARNING LI2 IS NULL')
            t2 = Trace(self.layers() , self.traceWidth(), self.net(), None, self.li2.line())
            # t2 = TraceItem(self.traceWidth(), self.layer(), self.li2.line())
        ...
        if (self.l3 is not None) and (not self.l3.isNull()) and (self.li3.isVisible()):
            t3 = Trace(self.layers(), self.traceWidth() , self.net(), None, self.li3.line())
            # t3 = TraceItem(self.traceWidth() , self. layer() , self.li3.line())
...
# Dont remove self.t0, if we only did a press-release
    ...
    @classmethod
    def fromPoints(cls, layers, traceWidth, p1, p2): 
        return cls( p1, p2, layers=layers, traceWidth=traceWidth )
